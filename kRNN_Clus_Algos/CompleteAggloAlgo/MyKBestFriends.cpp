...
bool MyKBestFriends::FindFriends(DataSet& ds,unsigned int thisk)
{
	...
	fprintf(this->m_summary_fp, " Each data point in your input dataset is labelled sequentially starting with first point labelled as zero.<br>\n");
	fprintf(this->m_summary_fp, "No of points in dataset: %d<br>\n", ds.GetNoOfPoints());
	fprintf(this->m_summary_fp, "No of dimensions: %d<br>\n", ds.GetNoOfDimensions());
	fprintf(this->m_summary_fp, "<hr><br><br>\n");
...
bool MyKBestFriends::FormClusters(DataSet& ds, unsigned int kvalue)
{
	...
	fprintf(this->m_summary_fp, "Solution at k=%d<br><br>\n", kvalue);
	this->m_kiter = kvalue;
	...
	fprintf(this->m_summary_fp,"No of outliers at k=%d : %d<br>\n", this->m_kiter, tempcnt);
	for(i = 0; i < ds.GetNoOfPoints(); i ++)
		this->m_finishedtimesinDFS[i] = 0;
	...
	free(arr_index);
	arr_index = NULL;
	fprintf(this->m_summary_fp, "No of clusters formed: %d<br>\n", this->m_noOfClusters);
	...
	while(this->m_noOfClusters >= 1)
	{
		...
		for(int i = 0; i < this->m_noOfClusters; i ++)
		{
			...
			for(int j = 0; j < ds.GetNoOfPoints(); j ++)
			{
				if(this->m_membersInClusters[j] == i)
					//fprintf(fp, "%d ", j);
					fprintf(fp, "%s ", ds.GetPoint(j).GetName());
			}
			fprintf(fp, "\n");
		}
		fclose(fp);
		...
		if(ds.GetNoOfDimensions() < 4)
		{
			...
			for(int i = 0;i < this->m_noOfClusters;i ++)
			{
				...
				if(cnt == 0)
				{
					...
					else if(ds.GetNoOfDimensions() == 3)
						fprintf(commandfp, "splot '%s'", filename);
					else
						fprintf(commandfp, "plot '%s'", filename);
				...
				if(ds.GetNoOfDimensions() == 3)
				{
					for(int j=0; j < ds.GetNoOfPoints(); j++)
						if(this->m_membersInClusters[j] == i)
							fprintf(newfp, "%lf\t%lf\t%lf\n", ds.GetPoint(j).GetDims(0), ds.GetPoint(j).GetDims(1), ds.GetPoint(j).GetDims(2));
				}
				else 
				{
					for(int j=0; j < ds.GetNoOfPoints(); j++)
			...
			system(filename);
			sprintf(filename, "convert %s/level%d.png %s/level%d.jpg", dirname, level, dirname, level);
			system(filename);
			fprintf(this->m_summary_fp, "<img src=\"%s/level%d.jpg\"></img><br>\n", dirname, level);
			//fprintf(this->m_summary_fp, "<a href=\"%s/levels/level%d.txt\">Clusters txt file</a><br><br>", dirname, level);
			fprintf(this->m_summary_fp, "End of level %d at value of k=%d<br><hr><br>\n", level, this->m_kiter);
		}
		if(this->m_noOfClusters == 1)
			break;
		
		checked = (int *)malloc(sizeof(int)*this->m_noOfClusters);
		clussim = (int **)malloc(sizeof(int *)*this->m_noOfClusters);
		for(int i = 0; i < this->m_noOfClusters; i ++)
			clussim[i] = (int *)malloc(sizeof(int)*this->m_noOfClusters);
		noofchecked = 0;
		...
		for(int i = 0; i < this->m_noOfClusters; i ++)
		{
			for(int j = 0; j < this->m_noOfClusters; j ++)
			{
				if(i != j)
				{
					fromitoj = this->m_k;
					fromjtoi = this->m_k;
					for(int k = 0; k < ds.GetNoOfPoints(); k ++)
					{
						if(this->m_membersInClusters[k] == i)
						{
							for(int l = 0; l < this->m_k; l ++)
							{
								int flag=0;
								nooffriends = this->m_indvfriends[k].GetNoOfFriendsOfDegree(l);
								friends = this->m_indvfriends[k].GetFriendsOfDegree(l);
								for(int m = 0; m < nooffriends; m ++)
								{
									if(this->m_membersInClusters[friends[m]] == j)
									{
										if(fromitoj > l)
											fromitoj = l;
										flag = 1;
						...
						else if(this->m_membersInClusters[k] == j)
						{
							for(int l = 0; l < this->m_k; l ++)
							{
								int flag=0;
								...
								for(int m = 0; m < nooffriends; m ++)
								{
									if(this->m_membersInClusters[friends[m]] == i)
									{
										if(fromjtoi > l)
											fromjtoi = l;
										flag = 1;
					...
					clussim[i][j] = fromitoj>fromjtoi?fromitoj:fromjtoi;
				}
				else
				{
					clussim[i][j] = INT_MAX;
		...
		for(int i = 0; i < this->m_noOfClusters; i ++)
		{
			for(int j = 0; j < this->m_noOfClusters; j ++)
			{
				if(clussim[i][j] < min && clussim[i][j] > this->m_kiter)
				{
					min = clussim[i][j];
					min_j = j;
					min_i = i;
		...
		if(clussim[min_i][min_j] == INT_MAX)
			break;
		...
		int outcnt=0;
		int tresh = (int)(float)min/(float)ds.GetNoOfDimensions();
		for(int i = 0; i < ds.GetNoOfPoints(); i ++)
		{
			for(int j = 0; j < this->m_noOfClusters; j ++)
				clusvalues[j] = 0;
